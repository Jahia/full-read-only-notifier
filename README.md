...
```bash
yarn install
yarn webpack          # development build (watch)
yarn build:production # production build
```

## Testing

End-to-end tests are located in the `tests/` directory and run with Cypress inside Docker.

### Test setup

```bash
cd tests
cp .env.example .env   # then fill in JAHIA_IMAGE, JAHIA_LICENSE, etc.
yarn install
```
...
```bash
# Start Jahia + run Cypress (Docker)
source ./set-env.sh && ./ci.startup.sh

# Open Cypress interactively (local)
yarn e2e:debug

# Run headless (local, Jahia must be running)
yarn e2e:ci
```
...
| Spec | What it covers |
|---|---|
| `01-fronotifierSettings.cy.ts` | Admin UI rendering, save/cancel, GraphQL persistence, content reloads |
| `02-fronotifierPopup.cy.ts` | Notification display, cookie lifecycle, mode transitions via Groovy scripts, dismiss button |

### Reports

```bash
yarn report:merge   # merge per-spec JSON files
yarn report:html    # generate HTML report
```
